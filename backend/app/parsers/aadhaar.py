...
from app.parsers.base import DocumentPlugin, FieldResult, ParsedDocument, DocumentSchema
...
class AadhaarPlugin(DocumentPlugin):
    document_id = "aadhaar_card"
    display_name = "Aadhaar Card"
    aliases = ["aadhaar"]
    supported_sides = ["front"]
    schema = DocumentSchema(
        expected_fields=["name", "dob", "gender", "aadhaar_number"],
    ...
    def _aadhaar_name_heuristic(self, ocr_lines: List[OCRLine]) -> FieldResult:
        ...
        for line in ocr_lines:
            ...
            if re.match(r'^\d{4}', text) or re.search(r'\b\d{4}\s+\d{4}\s+\d{4}\b', text): continue
            if re.search(r'(government|india|\u092d\u093e\u0930\u0924|\u0938\u0930\u0915\u093e\u0930|uidai|aadhaar|download|issue|vid|dob|\u091c\u0928\u094d\u092e|male|female|\u092a\u0941\u0930\u0941\u0937|\u092e\u0939\u093f\u0932\u093e|\bter\b|\bhteit\b)', text, re.I): continue
            if _RELATION_PREFIX_RE.match(text): continue
            if _AADHAAR_BACK_MARKER_RE.search(text): continue
...
AadhaarParser = AadhaarPlugin
document_registry.register(AadhaarPlugin())
